...
팀원 작업 환경은 유선 LAN, 개발 컴퓨터는 무선 Wi-Fi이므로 `10.1.92.119:1521`에 직접 도달 불가. 추가로 `finance_db.py`에 코드 버그도 있어 연결 가능 환경에서도 오작동. 두 가지를 동시에 수정한다.
...
ssh -N -L 1521:10.1.92.119:1521 <팀원_USER>@<팀원_PC_IP>
```

터널이 열리면 본인 맥북의 `localhost:1521` → 팀원 PC 경유 → `10.1.92.119:1521` 로 연결됨.

터널이 열리면 `localhost:1521` → `10.1.92.119:1521` 로 투명하게 연결됨.
...
터널 없이 LAN에 직접 접속할 때는 다시 `10.1.92.119`로 복원.
...
- 터널 없이 배포 환경(Azure Container Apps)에서 실행될 때는 `ORACLE_HOST=10.1.92.119` 유지
- `.env`는 절대 커밋 금지
