...
두 서버 모두 **사내 Oracle DB** (`10.1.92.119:1521/xe`)에 접속합니다.  
...
DB_INFO = "shobi/8680@//10.1.92.119:1521/xe"  # ← 사내 IP, Azure에서 접근 불가
```
...
3. Oracle DB 서버(`10.1.92.119`)가 Azure에서 접근 가능한 프라이빗 IP로 라우팅되는지 확인  
...
_host     = os.getenv("ORACLE_HOST", "10.1.92.119")
...
| 변수명 | 설명 | 예시 |
|--------|------|------|
| `ORACLE_HOST` | Oracle 서버 호스트 | `10.1.92.119` 또는 Azure VM IP |
