...
export default function MapView() {
  ...
  const { allStoresRef, drawMarkers, clearMarkers, selectMarker, highlightById, markerLayerRef } =
  ...
  const handleFindAndHighlightByName = useCallback(
    async (name) => {
  ...
  const handleSearch = async (query) => {
    ...
    if (!matched.length) {
      try {
        ...
        if (jj.data?.length) {
          ...
          matched = features.filter((f) => admCds.has(f.getProperties().adm_cd));
        }
      } catch (e) {
    ...
    if (storeSearchOn) {
      ...
      setNearbyCount(null);
      const admCds = [
        ...new Set(matched.map((f) => (f.getProperties().adm_cd || "").trim()).filter(Boolean)),
      ];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const r = await fetch(`/kakao/v2/local/geo/coord2regioncode.json?x=${cLng}&y=${cLat}`, {
          headers: { Authorization: `KakaoAK ${KAKAO_REST_KEY}` },
        });
        ...
        onBack={
          prevStorePopupRef.current
            ? () => {
                ...
                lastClusterStoresRef.current = prev.clusterStores || null;
                prevStorePopupRef.current = null;
              }
            : null
        }
      ...
      <StorePopup
        ...
        onLandValue={() => {
          ...
          if (map) {
            ...
            const doWmsClick = () => {
              handleWmsClick(map, coord, { skipZoomGuard: true }).then((result) => {
                ...
                if (pnu) {
                  ...
                  fetch(`${REALESTATE_URL}/realestate/land-value?pnu=${encodeURIComponent(pnu)}`, {
                    headers: _mapHeaders,
                  })
                    .then((r) => r.json())
                    .then((lv) => {
            ...
            if (currentZoom < 18) {
              map.getView().animate({ center: coord, zoom: 18, duration: 500 }, doWmsClick);
            } else {
              doWmsClick();
            }
          }
