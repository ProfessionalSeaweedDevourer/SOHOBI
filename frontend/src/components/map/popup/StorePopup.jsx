    ...
    >
      ...
      <div className="sp-store-info">
        ...
        <div className="sp-store-meta">
          {c.label}
          {variant === "list" && store.ADM_NM ? ` · ${store.ADM_NM}` : ""}
        </div>
      </div>
    </div>
  );
...
function ClusterListView({ stores, onSelect, onClose }) {
  ...
  const filtered = filter
    ? stores.filter((s) => s.STORE_NM?.includes(filter) || s.CAT_NM?.includes(filter))
    : stores;
  ...
  return (
    <div className="sp-body">
      ...
      <div className="sp-scroll">
        ...
        {filtered.length === 0 && <div className="sp-empty">검색 결과 없음</div>}
      </div>
    </div>
  );
}
    ...
    <div className="sp-body">
      <div className="sp-top">
        <div className="sp-top-left">
          {onBack && (
            <button onClick={onBack} className="sp-back-btn">
              ←
            </button>
          )}
          <div
      ...
      {loadingDetail && <div className="sp-loading">📱 카카오맵 상세정보 조회 중...</div>}
        ...
        <>
          <div className="sp-kakao-box">
            ...
            {kakaoDetail.category_name && (
              <div className="sp-row">
                <span className="sp-row-icon">🏷️</span>
                <span className="sp-row-text">{kakaoDetail.category_name}</span>
              </div>
            )}
          </div>
      ...
      {nearbyStores.length > 0 && (
        <>
          ...
          <div className="sp-nearby-title">같은 건물 상가 ({nearbyStores.length}건)</div>
          <div className="sp-nearby-list">
            {nearbyStores.slice(0, 20).map((s, i) => (
              <StoreItem key={`${s.STORE_ID || "x"}-${i}`} store={s} onClick={onStoreSelect} />
            ))}
          </div>
        </>
      )}
  ...
  if (clusterStores && clusterStores.length > 0 && !popup) {
    return (
      <div className={popupClass}>
        ...
        <ClusterListView stores={clusterStores} onSelect={onClusterSelect} onClose={onClose} />
      </div>
    );
  }
