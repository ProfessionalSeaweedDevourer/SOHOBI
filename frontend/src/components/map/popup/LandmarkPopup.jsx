      ...
      <div style={{ padding: "12px 16px 16px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          {(popup.addr || popup.road_addr || popup.addr1) && (
            <Row
              icon="📍"
              text={[popup.addr || popup.road_addr || popup.addr1, popup.addr2]
                .filter(Boolean)
                .join(" ")}
            />
          )}
          {isSchool && popup.edu_office && <Row icon="🏛️" text={popup.edu_office} />}
          {popup.tel && (
            <Row
              icon="📞"
              text={
                <a href={`tel:${popup.tel}`} style={{ color: "#2563eb", textDecoration: "none" }}>
                  {popup.tel}
                </a>
              }
            />
          ...
          {popup.homepage && (
            <Row
              ...
              text={
                <a
                  href={
                    popup.homepage.startsWith("http") ? popup.homepage : `http://${popup.homepage}`
                  }
                  target="_blank"
                  rel="noreferrer"
                  style={{
                ...
                >
                  {popup.homepage.replace(/^https?:\/\//, "").slice(0, 40)}
          ...
          {popup.start_date && (
            <Row icon="📅" text={`${popup.start_date} ~ ${popup.end_date || ""}`} />
          )}
