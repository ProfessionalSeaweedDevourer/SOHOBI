...
export default function DongPanel({
  ...
  return (
    <div className="mv-dong-panel">
      <div className={`mv-dong-panel__header ${panelClass}`}>
      ...
      {quarters.length > 0 && !isRE && !isStore && (
        ...
        >
          ...
          {/* 년도 선택 */}
          <select
            value={selectedQuarter ? String(selectedQuarter).slice(0, 4) : ""}
            onChange={(e) => {
              const yr = e.target.value;
              // 해당 년도의 첫 번째 분기 자동 선택
              const first = quarters.find((q) => String(q).startsWith(yr));
              ...
              .filter((q) =>
                String(q).startsWith(
                  selectedQuarter
                    ...
                    : String(quarters[quarters.length - 1]).slice(0, 4),
                ),
              )
              .map((q) => (
