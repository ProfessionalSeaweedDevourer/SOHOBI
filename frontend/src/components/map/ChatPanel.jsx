    ...
    async (directText) => {
      ...
      if (navMatch) {
        ...
        if (moved) {
          setMessages((prev) => [
            ...prev,
            { id: crypto.randomUUID(), role: "system", content: `${placeName}(으)로 지도를 이동했습니다.` },
          ]);
          return;
        }
        if (onSearchArea) {
          ...
          setMessages((prev) => [
            ...prev,
            { id: crypto.randomUUID(), role: "system", content: `지도에서 "${placeName}" 검색 중입니다.` },
          ]);
          return;
        }
        setMessages((prev) => [
          ...prev,
          { id: crypto.randomUUID(), role: "system", content: `지도 이동 기능을 사용할 수 없습니다. 상권 분석은 "홍대 카페 분석" 형식으로 입력하세요.` },
        ]);
        return;
      }
      ...
      try {
        await streamQuery(question, 3, sessionId, (eventName, data) => {
          ...
          if (eventName === "chunk") {
            ...
            setMessages((prev) =>
              prev.map((m) => (m.id === streamMsgId ? { ...m, content: draft, charts, suggestedActions } : m)),
            );
            ...
            setMessages((prev) =>
              prev.map((m) => (m.id === streamMsgId ? { ...m, content: msg } : m)),
            );
          }
        });
      } catch (err) {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === streamMsgId ? { ...m, content: `오류가 발생했습니다: ${err.message}` } : m,
          ),
        );
      } finally {
        setLoading(false);
    ...
    <>
      ...
      <div className={`mv-chat-panel ${isOpen ? "" : "mv-chat-panel--closed"} ${dongPanelOpen ? "mv-chat-panel--dong-open" : ""}`}>
        ...
        <div className="mv-chat-header">
          ...
          <button className="mv-chat-header__close" onClick={onToggle} title="패널 닫기">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="mv-chat-messages">
          {messages.length === 0 && (
            <div className="mv-chat-empty">
              ...
              <p className="mv-chat-empty__title">상권 분석을 시작하세요</p>
              <p className="mv-chat-empty__desc">
                지역과 업종을 입력하면 상권을 분석해드립니다.<br />
