...
const AREA_PATTERN = new RegExp(`(${AREA_NAMES.join("|")})(?=구|동|역|로|\\s|,|\\.|!|\\?|$)`, "g");
    ...
    () => ({
      p: ({ children }) => {
        ...
        const processed = toArr.flatMap((child, i) =>
          typeof child === "string"
            ? renderWithAreaLinks(child, onFindAndHighlightByName, `p-${i}`)
            : [child],
        );
      ...
      li: ({ children }) => {
        ...
        const processed = toArr.flatMap((child, i) =>
          typeof child === "string"
            ? renderWithAreaLinks(child, onFindAndHighlightByName, `li-${i}`)
            : [child],
        );
      ...
      strong: ({ children }) => {
        ...
        const processed = toArr.flatMap((child, i) =>
          typeof child === "string"
            ? renderWithAreaLinks(child, onFindAndHighlightByName, `strong-${i}`)
            : [child],
        );
