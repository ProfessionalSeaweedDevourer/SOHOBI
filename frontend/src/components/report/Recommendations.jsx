...
const AGENT_FOR_ITEM = {
  biz_type: {
    label: "업종 상담",
    path: "/user",
    hint: "AI 에이전트에게 업종·업태 결정을 물어보세요",
    color: "#0891b2",
  },
  location: {
    label: "상권 분석",
    path: "/map",
    hint: "상권 지도에서 입지를 분석해 보세요",
    color: "#14b8a6",
  },
  capital: {
    label: "자금 계획",
    path: "/user",
    hint: "재무 에이전트로 초기 자금 계획을 세워보세요",
    color: "#f97316",
  },
  biz_reg: {
    label: "사업자 등록",
    path: "/user",
    hint: "법률 에이전트에게 사업자 등록 절차를 확인하세요",
    color: "#8b5cf6",
  },
  permit: {
    label: "영업 신고",
    path: "/user",
    hint: "행정 에이전트에게 영업 허가·신고를 문의하세요",
    color: "#0891b2",
  },
  labor: {
    label: "인력 채용",
    path: "/user",
    hint: "법률 에이전트에게 근로계약·4대보험을 확인하세요",
    color: "#ec4899",
  },
  finance_sim: {
    label: "수익성 검토",
    path: "/user",
    hint: "재무 에이전트로 손익 시뮬레이션을 실행해 보세요",
    color: "#f97316",
  },
  lease: {
    label: "임대차 계약",
    path: "/user",
    hint: "법률 에이전트에게 임대차 계약서 검토를 요청하세요",
    color: "#8b5cf6",
  },
};
...
export default function Recommendations({ incompleteItems, sessionId }) {
  ...
  const items = incompleteItems.slice(0, 3).map((id) => {
  ...
  return (
    ...
    >
      {/* 섹션 헤더 */}
      <div
        className="px-5 pt-5 pb-4 flex items-center gap-2 border-b"
      ...
      <div className="px-5 pb-5 pt-4 flex flex-col gap-3">
        {items.map((item, idx) => (
          <motion.a
            ...
            onClick={() =>
              trackEvent("report_recommendation_click", {
