...
| # | 소스 | API 키 환경변수 | 설명 |
|---|------|-----------------|------|
| 1 | 정부24 공공서비스 API | `GOV24_API_KEY` | 정부 보조금, 지원사업, 창업패키지 (35개 키워드 필터링) |
| 2 | K-Startup 사업공고 API | `KSTARTUP_API_KEY` | 창업진흥원 사업소개/공고 (data.go.kr #15125364) |
| 3 | 창업공간플랫폼 API | `KISED_SPACE_API_KEY` | 전국 창업공간/보육센터/코워킹스페이스 (data.go.kr #15125365) |
| 4 | 정부지원사업 주관기관 API | `KISED_AGENCY_API_KEY` | 지원사업별 수행기관 정보 (data.go.kr #15125366) |
| 5 | 창업에듀 교육과정 API | `KISED_EDU_API_KEY` | 온라인 창업교육 과정 (data.go.kr #15125358) |
| 6 | 중소벤처24 API | `SME24_API_KEY` | 중기부 지원사업 공고 |
| 7 | 기업마당 API | `BIZINFO_API_KEY` | 범정부 지원사업 통합 검색 |
| 8 | 큐레이션 데이터 | (키 불필요) | 소진공 정책자금, 신용보증, 고용지원, 외식업 특화 등 24건 |
