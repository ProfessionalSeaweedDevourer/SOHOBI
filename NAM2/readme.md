...
2. 환경변수 추가 (예: `KSTARTUP_API_KEY`, `SME24_API_KEY`, `BIZINFO_API_KEY`)
...
[Azure Functions - Timer/HTTP Trigger]
    |
    v
[데이터 수집] -- 다중 API 호출 + 큐레이션 데이터 병합
    +-- 정부24 API (GOV24_API_KEY)
    +-- K-Startup API (KSTARTUP_API_KEY) -- 키 있으면 자동 수집
    +-- 중소벤처24 API (SME24_API_KEY) -- 키 있으면 자동 수집
    +-- 기업마당 API (BIZINFO_API_KEY) -- 키 있으면 자동 수집
