...
[API Server] ── session_id 발급·복원 / 창업자 프로필 저장
    │            session_store.py (Cosmos DB / 인메모리 폴백)
    │
    ▼
[DomainRouter] ── 키워드(1차) / LLM(2차) 분류
    ...
    ├─ admin    → [AdminAgent]
    │             ├─ 창업자 프로필 시스템 프롬프트 주입
    │             ├─ AdminProcedurePlugin
    │             │    법령 검증 절차 KB (영업신고·위생교육·사업자등록·보건증·소방)
    │             ├─ SeoulCommercialPlugin
    │             │    서울시 상권 API — 지역·업종별 매출·점포수 자동 조회
    │             └─ GovSupportPlugin
    │                  정부지원사업 하이브리드 검색 RAG
    │                  (보조금·창업패키지·대출·신용보증·고용지원·교육컨설팅, 5600건+)
    │
    ├─ finance  → [FinanceAgent]  ← 4단계 명시적 파이프라인
    │             ├─ 창업자 프로필 → 파라미터 추출 + 설명 생성 단계 주입
    │             └─ FinanceSimulationPlugin
    │                  1) 자연어 → 시뮬레이션 파라미터 JSON 추출
    │                  2) 몬테카를로 10,000회 시뮬레이션
    │                     (평균·표준편차·P5·P20·P95·손실확률 + 히스토그램 차트)
    ...
    │             ├─ 창업자 프로필 시스템 프롬프트 주입
    │             └─ LegalSearchPlugin
    │                  Azure AI Search 법령 RAG (벡터 검색, top-3)
    │
    ├─ location → [LocationAgent]  ← DB 조회 + LLM 분석 파이프라인
    │             ├─ 자연어 → {mode, locations, business_type, quarter} 파라미터 추출
    │             └─ CommercialRepository (Azure PostgreSQL)
    │                  1) 단일 지역: 월매출·점포수·개폐업률·시간대·성별·연령대 조회
    │                     → LLM 분석 (상권별 분리 분석 + 기회·리스크)
    │                     → 유사 상권 추천 테이블 첨부
    │                  2) 복수 지역: 지역별 지표 비교표
    │                     → LLM 창업 추천 순위 생성
    │
    └─ chat     → [ChatAgent]  ← Sign-off 바이패스, 즉시 반환
                  서비스 안내, 에이전트 사용법, 일상 대화 전담
                  플러그인 없음
    │
    ▼ (chat 도메인 제외)
[SignOffAgent] ── 도메인별 루브릭 판정
...
- Azure PostgreSQL Flexible Server (상권 분석 데이터)
...
| `domain` | ⬜ | `"admin"` / `"finance"` / `"legal"` / `"location"` / `"chat"`. `null`이면 자동 분류 |
...
# 안내 도메인 (Sign-off 없이 즉시 응답)
curl -s -X POST http://localhost:8000/api/v1/query \
  -H "Content-Type: application/json" \
  -d '{"question":"상권 분석 에이전트 어떻게 사용하나요?"}'
```
...
| ------ | ------------ | -------------------- |
| `admin` | 영업 신고, 허가, 행정 절차, 정부지원사업 | AdminProcedurePlugin, GovSupportPlugin, SeoulCommercialPlugin |
| `finance` | 매출 시뮬레이션, 투자 분석, 수익성 | FinanceSimulationPlugin |
| `legal` | 법령 해석, 계약, 임대차, 권리 의무 | LegalSearchPlugin |
| `location` | 상권 분석, 지역 비교, 매출 지역 데이터, 창업 입지 | CommercialRepository (Azure PostgreSQL) |
| `chat` | 서비스 안내, 에이전트 사용법, 일상 대화 | 없음 (Sign-off 바이패스) |
...
├── map_router.py                 # 지도 데이터 API 라우터 (행정동 매출·점포수, PostgreSQL)
...
│   ├── seoul_commercial_plugin.py   # 서울시 상권 API
