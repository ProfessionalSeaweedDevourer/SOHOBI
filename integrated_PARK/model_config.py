...
# ┌──────────────┬───────────────────────────────────────────────────────────┐
# │ gpt-5.4-mini │ 극속·저비용 — 라우팅·FAQ·파라미터 추출 (모든 티어 고정)    │
# │ gpt-5.4      │ GPT 표준 — 무료 티어 핵심 추론 모델                       │
...
MODEL_CONFIG = {
    "free": {
        ...
        "admin":           "gpt-5.4",       # 3개 플러그인 통합·법령 검증
        "finance_param":   "gpt-5.4-mini",  # JSON 파라미터 추출 — 구조화 입력 파싱
        "finance_explain": "gpt-5.4",       # 시뮬레이션 결과 해설
        "location":        "gpt-5.4",       # DB 결과 분석·상권 해설
        "legal":           "gpt-5.4",       # RAG 법령 인용·계약 해석
        "signoff":         "gpt-5.4",       # JSON 루브릭 판정 (구조화 출력)
    },
    "premium": {
