...
| 파일 | 도메인 | 역할 | 플러그인 | 담당 |
|------|--------|------|----------|------|
| `admin_agent.py` | `admin` | 행정 절차 안내 + 정부지원사업 추천 | AdminProcedurePlugin, GovSupportPlugin | NAM |
...
| `chat_agent.py` | `chat` | 서비스 안내·일상 대화 | 없음 | PARK |

## 공통 동작

1. **창업자 프로필 주입** — 세션에 저장된 `founder_context`(업종, 지역, 예산 등)를 시스템 프롬프트에 삽입
2. **SSE 스트리밍** — `orchestrator.run_stream()`을 통해 토큰 단위 스트리밍 응답 지원
3. **Sign-off 검증** — 5개 에이전트 모두의 응답은 [`signoff/`](../signoff/)에서 품질 판정 (`chat` 도메인은 `CH1~CH5` 루브릭 적용)
...
2. **LLM 분류** — 키워드 매칭 실패 시 LLM(`router` 서비스) JSON 분류 (fallback: `admin`)
