...
_EXPLAIN_PROMPT = """아래 구조에 맞춰 창업자에게 직접 말하는 형식으로 답변을 작성하십시오.
질문을 되풀이하거나 '질문'·'응답' 같은 역할 라벨을 출력하지 말고, [1], [2], [3], [안내] 섹션 구조만 사용합니다.
    ...
    async def _call_llm_with_history(
        ...
        try:
            ...
            if "content_filter" in err_str or "content filter" in err_str:
                ...
                result = await asyncio.wait_for(
                    service.get_chat_message_content(history2, settings=settings),
                    timeout=60.0,
                )
                return result.content or str(result)
            raise ValueError(f"AI 응답 생성 중 오류가 발생했습니다: {e}") from e
