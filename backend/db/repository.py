...
AREA_MAP = {
    ...
    # 랜드마크·역명 별칭 (사용자 입력 매칭률 향상)
    "강남역": ["11680640", "11680650"],
    "홍대입구": ["11440660"],
    "합정역": ["11440680"],
    "신촌역": ["11410585"],
    "이대": ["11410585"],
    "건대입구": ["11215710"],
    "서울역": ["11170510", "11140680"],
    "신사": ["11680521", "11680531"],
    "가로수길": ["11680521", "11680531"],
    "경리단길": ["11170650", "11170660"],
    "해방촌": ["11170650", "11170510"],
    "연트럴파크": ["11440710"],
    "망리단길": ["11440690", "11440700"],
    "북촌": ["11110540", "11110600"],
    "대학로": ["11110650"],
    "익선동": ["11110615"],
    "상수": ["11440660", "11440680"],
    "서교": ["11440660"],
...
class CommercialRepository:
    ...
    def _lookup_adm_codes_by_name(self, adm_nm: str) -> list[str]:
        ...
        sql_exact = (
            "SELECT DISTINCT adm_cd FROM sangkwon_sales WHERE adm_nm = %s LIMIT 5"
        )
        sql_like = (
            "SELECT DISTINCT adm_cd FROM sangkwon_sales WHERE adm_nm LIKE %s LIMIT 5"
        )
        conn = self._connect()
        try:
    ...
    def get_similar_locations(
        ...
        norm_close = normalize(
            [float(r["close_rate"] or 0) for r in rows], inverse=True
        )
